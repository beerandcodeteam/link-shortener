@props([
    'mode' => 'guest', // 'guest' | 'app'
])

{{-- Sticky glassmorphic NavBar — shared partial used by both guest and app layouts. --}}
<nav class="nav sticky top-0 z-50 h-[52px] bg-white/72 backdrop-blur-xl border-b border-line-soft"
     x-data @click.outside=""
>
    <div class="nav-inner max-w-[1080px] mx-auto h-full px-7 flex items-center gap-5">

        {{-- Logo — always visible, targets home or dashboard based on mode --}}
        @if ($mode === 'app')
        ...
        @else
            <a href="{{ route('home') }}" class="logo inline-flex items-center gap-2 font-semibold text-lg tracking-tight text-ink cursor-pointer" aria-label="Snip home">
                <span class="logo-mark w-[26px] h-[26px] grid place-items-center">
                    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M8.5 3v4a1 1 0 0 1-1 1h-4m13-5h5m-5 14h5m-5 4v3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        <path d="M21.5 10.5L10.5 21.5" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
                </span>
                Snip
            </a>
        @endif

        {{-- Contextual links — different sets for guest vs authenticated --}}
        ...
        <div class="flex items-center gap-2">
            @if ($mode === 'app')
                {{-- Authenticated: "New link" CTA + avatar dropdown --}}
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    <button @click="open = !open" type="button" aria-expanded="false" aria-haspopup="true"
                            class="avatar w-8 h-8 rounded-full bg-blue-tint flex items-center justify-center text-blue font-semibold text-sm cursor-pointer hover:border-blue/30 transition-colors focus:outline-none focus:ring-2 focus:ring-blue/50">
                    ...
                    <div x-show="open"
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" role="menuitem" class="w-full text-left px-4 py-2 text-sm text-ink-2 hover:text-red hover:bg-red-tint cursor-pointer">
                                Sign out
                            </button>
                        </form>
                    </div>
